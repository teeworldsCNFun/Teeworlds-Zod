...
{
	if(!m_apPlayers[ClientID]->GetZomb())
	{
...
void CGameContext::OnTick()
{
	...
	if(m_VoteCloseTime)
	{
		...
		else
		{
			...
			{
				// count votes
				char aaBuf[4][NETADDR_MAXSTRSIZE] = {{0}};
				for(int i = 0; i < 16; i++)
					if(m_apPlayers[i])
				...
				for(int i = 0; i < 16; i++)
				{
					if(!m_apPlayers[i] || (m_apPlayers[i]->GetTeam() == TEAM_SPECTATORS && !g_Config.m_SvAllowSpecVote) || aVoteChecked[i])	// don't count in votes by spectators
						continue;
...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	if(MsgID == NETMSGTYPE_CL_SAY)
	{
		...
		if(!str_comp(pMsg->m_pMessage, "/cmdlist") || !str_comp(pMsg->m_pMessage, "!cmdlist"))
		{
			CNetMsg_Sv_Motd Msg;
			Msg.m_pMessage = "~~~Commands~~~ \n/info or /help for Informations \n/top5 for the Top-5 Teams \n/restart or !restart to start a restart vote! \nDo you know all Types of zombies?\nPlay to find out! \n \nI thought it would be good \nto make a harder zombie mod :D";
			Server()->SendPackMsg(&Msg, MSGFLAG_VITAL, ClientID);
			m_MessageReturn = 250;
			return;
		}
		...
		else if(!str_comp(pMsg->m_pMessage, "/top5"))
		{
			...
			else
			{
				...
				for(int i = 0; i < 5; i++)//top 5 TEAMS
				{
					...
					{
						//if(m_pTop->m_TopFiveVars.m_aTeams[i].m_aaName[p] == '\0' || m_pTop->m_TopFiveVars.m_aTeams[i].m_aaName[p] == "")
							//continue;
