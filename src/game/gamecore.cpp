...
void CCharacterCore::Tick(bool UseInput)
{
	...
	else if(m_HookState == HOOK_FLYING)
	{
		...
		if(m_pWorld && m_pWorld->m_Tuning.m_PlayerHooking && !g_Config.m_SvLongFly)
		{
			float Distance = 0.0f;
	...
	if(m_pWorld)
	{
		for(int i = 0; i < MAX_CLIENTS; i++)
		{
			...
			if(m_pWorld->m_Tuning.m_PlayerCollision && !g_Config.m_SvLongFly && Distance < PhysSize*1.25f && Distance > 0.0f)
			{
			...
			if(m_HookedPlayer == i && m_pWorld->m_Tuning.m_PlayerHooking && !g_Config.m_SvLongFly)
			{
...
void CCharacterCore::Move()
{
	...
	if(m_pWorld && m_pWorld->m_Tuning.m_PlayerCollision && !g_Config.m_SvLongFly)
	{
